...
class LambdaDataset:
    ...
    def lambda_pre_process(self, dataframe, optic):
        ...
        for i, j in enumerate(
            dataframe.index
        ):  # row boyunca döngüye sokar her satır numune için yukarıdaki metodu uygular
            optic_list = optic_list + list(
                dataframe.iloc[i, optic_first:optic_last].values
            )
    ...
    def create_lambda_dataset(self, pre_only=True):
        dataframe = self.dataframe
        if pre_only == True:
            column_name_list = ["T_pre", "Rc_pre", "Ru_pre"]
        else:
            column_name_list = [
                "T_pre",
                "Rc_pre",
